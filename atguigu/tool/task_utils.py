...
def create_q(task_id):
    if not q_dict.get(task_id):
        q_dict[task_id] = Queue()
    return q_dict[task_id]


def put_q(task_id, event, data):
    while not q_dict.get(task_id):
        time.sleep(1)
    q = q_dict.get(task_id)
